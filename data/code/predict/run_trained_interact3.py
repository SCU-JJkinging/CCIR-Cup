...
from data.code.model.InteractModel_3 import InteractModel
...
def find_slot(tokenizer, input_ids_list, slot_pre_output, ori_sen_list):
    ...
    for i, slot_ids in enumerate(slot_pre_output):  # 遍历每条数据
        ...
        if is_nest(slot_ids[1:-1]):  # 如果确实存在嵌套行为
            ...
            slot_list.append(tmp_dict)
    print(str(count)+'——interact3 推理完成！')
    fp.close()
    return slot_list
...
if __name__ == "__main__":
    ...
    model = InteractModel('../../user_data/pretrained_model/ernie',
                          ...
                          slot_tag_size,
                          device).to(device)
    checkpoint = torch.load('../../user_data/output_model/InteractModel_3/trained_model/Interact3_model_best.pth.tar')
    ...
    with open('../../user_data/tmp_result/result_interact_3.json', 'w', encoding='utf-8') as fp:
        json.dump(res, fp, ensure_ascii=False)

    source_path = '../../user_data/tmp_result/result_interact_3.json'
    target_path = '../../user_data/tmp_result/result_interact_3_post.json'
